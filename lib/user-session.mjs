...
export class UserSession extends Container {
    constructor({ username, sessionAuthToken, hashedPassphrase, passphrase, storeAuthToken }) {
        username = username ? username.toLowerCase() : 'anonymous';
            ...
            root: {
                container: {
                    members: {
                        ...
                        setup: {
                            callback: async () => {
                                const _username = await this.username;
                                ...
                                if (_username && (await store.exists())) {
                                    let content = await store.read();
                                    content = utils.getJSONObject(content);
                                    const userInfo = content[_username];
                                    if (userInfo) {
                                        ({ storedHashedPassphrase, storedHashedPassphraseSalt, storedDecryptionKey, storedEncryptionKey } = userInfo);
                                        if (sessionAuthToken) {
                                            sessionAuth = utils.base64ToString(sessionAuthToken);
                                            sessionAuth = JSON.parse(sessionAuth);
                                        }
                                    } else {
                                        throw new Error(`${_username} was not in the store`);
                                    }
                                }
                                users.set(_username, {
                                    sessionAuth,
