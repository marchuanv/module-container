...
export class Reference {
   ...
   addReference(referenceId, prototype) { //has to be an existing reference
      ...
      if (referenceId.prototype === prototype.constructor) {
         ...
         if (Object.getPrototypeOf(referenceId.prototype) === prototype.constructor) {
            ...
            throw new Error(`${referenceId.prototype.name}(${referenceId.Id.name}) reference is not a ${_prototype.name} class`);
         }
      }
      if (!_references.has(_prototype)) {
         throw new Error(`found no references for ${_prototype} prototype`);
      }
      const { referencesRefId } = _references.get(_prototype).find(x => x.Id === referenceId);
      const references = _references.get(referencesRefId);
      if (references.has(this.Id.prototype)) {
         if (!references.get(this.Id.prototype).find(x => x === this.Id)) {
            references.get(this.Id.prototype).push(this.Id);
         }
      } else {
         references.set(this.Id.prototype, [this.Id]);
      }
      {
   ...
   */
   ...
   getReference(prototype) {
      const { Id } = _references.get(prototype.constructor).find(x => _references.get(x.referencesRefId).get(this.Id.prototype).find(refId => refId === this.Id));
   ...
   */
   getAllReferences(prototype) {
      return _references.get(prototype.constructor).filter(x => _references.get(x.referencesRefId).get(this.Id.prototype).find(refId => refId === this.Id))
         .map(x => {
