...
export class Reference {
   ...
   constructor(name) {
      ...
      _references.set(referenceId, { object: this, Class, dependantIds: [], data: {} });
   ...
   */
    ...
    */
   get(prototype) {
      const stack = utils.getJSONObject(utils.getJSONString(this.referencedIds));
      let refId = stack.shift();
      while (refId) {
         const { object, type } = _references.get(refId);
         if (object instanceof prototype && prototype === type) {
            return object;
         }
         refId = stack.shift();
      }
