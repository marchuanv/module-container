...
export class GithubFile extends Container {
   ...
   async getMetadata() {
      try {
         ...
         const response = await this.github.request({ path: `GET /repos/marchuanv/active-objects/contents/${_fileName}?ref=${this.branchName}` });
         const { data } = response;
         return data;
      } catch (error) {
   ...
   async ensureContent() {
      try {
         ...
         await this.github.request({
            path: `PUT /repos/marchuanv/active-objects/contents/${_fileName}`, options: {
               owner: 'marchuanv',
               repo: 'active-objects',
               path: `/${_fileName}`,
   ...
   async delete() {
      try {
         ...
         if (metadata) {
            await this.github.request({
               path: `DELETE /repos/marchuanv/active-objects/contents/${_fileName}`, options: {
                  owner: 'marchuanv',
                  repo: 'active-objects',
                  path: `/${_fileName}`,
                  message: 'deleted',
