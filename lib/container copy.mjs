...
export class Container {
   constructor() {
      ...
      if (properties.find(prop => prop === 'Id')) { //create an Id getter property on the prototype if it does not exist.
   ...
   dependency(obj) {
      ...
      if (keys.length === 0 && typeof obj === 'function') {
         ...
         const thisId = utils.generateGUID();

         const dependencies = dependencyKeyValues.find(o => o.Id === Id && o.key === key);
         ...
         for (const key of keys) {
            ...
            const dependency = dependencyKeyValues.find(o => o.Id === Id && o.key === key);
            if (dependency) {
               if (dependency.type) {
               ...
               if (containerClass) {
                  dependencyKeyValues.push({ Id, key, value: ctorArgs, type: value, changed: true });
               } else {
                  dependencyKeyValues.push({ Id, key, value, type: null, changed: true });
               }
            }
         }
...
const createPrivateGetProperty = ({ name }) => {
   Object.defineProperty(context, name, { configurable: false, get: (args) => {
    
   }});
}

const createPrivateSetProperty = ({ name }) => {
   Object.defineProperty(context, name, { configurable: false, set: (args) => {
      
   }});
}

const createPrivateMethod = ({ name }) => {
   Object.defineProperty(context, name, { configurable: false, get: (args) => {
    
   }});
}

const validateCallstack = () => {
   const stack = (new Error()).stack.split("\n");
   const caller = stack[2].trim().split(" ")[1];
   if (caller === 'new') {
      return callback.call(context, args);
