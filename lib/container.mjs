...
const mapClassMembers = ({ config, context }) => {
   const _dependencyMembers = dependencyMembers.get(context.contextId);
   const configDictionary = getConfigWithTemplateRefs({ config });
   const typeDictionary = [];
   for (let itemIndex = 0; itemIndex < configDictionary.length; itemIndex++) {
      let { key, value } = configDictionary[itemIndex];
      if (key === 'type') {
         for (itemIndex = itemIndex + 1; itemIndex < configDictionary.length; itemIndex++) {
            ({ key, value } = configDictionary[itemIndex]);
            const valueKeys = Object.keys(value);
            if (valueKeys.length > 0) {
               for (const typeKey of valueKeys) {
                  const Class = value[typeKey];
                  const name = getTypeName({ Class });
                  typeDictionary.push({ key: name, value: Class });
               }
            } else {
               const Class = value[typeKey];
               const name = getTypeName({ Class });
   ...
   const args = {};
   // let { key, value } = configDictionary[itemIndex];
   // let hasCtorAndClassConfig = false;
   // if (key === 'args') {
   //    for (itemIndex = itemIndex + 1; itemIndex < configDictionary.length; itemIndex++) {
   //       const { key, value } = configDictionary[itemIndex];
   //       if (key === 'type') {
   //          break;
   //       }
   //       args[key] = value;
   ...
   //    const member = new Member(name, Class, args, false, null, context);
   //    _dependencyMembers.push(member);
   //    hasCtorAndClassConfig = true;
   //    const member = new Member(name, Class, args, false, null, context);
   //    _dependencyMembers.push(member);
   //    hasCtorAndClassConfig = true;
   // } const member = new Member(name, Class, args, false, null, context);
...
const getConfigWithTemplateRefs = ({ config }) => {
   const template = createOrderedKeyTemplate({ config });
   const references = Object.keys(template)
      .filter(key => !template[key].ref)
   ...
   //       createConfigDictionaryFromTemplate({ config: value, template, dictionary });
   //    } else {
   //       dictionary.unshift({ key, value });
   //    }
   // }
   return dictionary;
}
