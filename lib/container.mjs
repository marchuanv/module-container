...
const mapClassMembers = ({ config, context, template }) => {
   const dictionary = getConfigDictionary({ config });
   for (const key of Object.keys(template)) {
      const value = template[key];
      const isEnd = /^[{]?[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}[}]?$/g.test(value);
      if (typeof value === 'object' && !isEnd) {
         mapClassMembers({ config: config[key], context, template: template[key] });
      } else if (isEnd) {
         const configItem = config[key];
         const classes = [];
         for (const key of Object.keys(configItem)) {
            const Class = configItem[key];
            if (Class.name) {
...
const getConfigDictionary = ({ config, template, parentName, depth = 0 }) => {
   if (!template) {
   ...
   for (const key of Object.keys(template)) {
      const value = config[key];
      const tempValue = template[key];
      if (typeof tempValue === 'object') {
         if (Object.keys(tempValue).length > 0) {
            const children = getConfigDictionary({ config: value, template: tempValue, parentName: key, depth: depth + 1 });
            const Id = utils.generateGUID();
            dictionary.push({ key, value: children, depth, Id });
         } else {
            const Id = utils.generateGUID();
            dictionary.push({ key: parentName, value: { key, value }, depth, Id });
         }
      } else {
         const Id = utils.generateGUID();
         dictionary.push({ key: parentName, value: { key, value }, depth, Id });
