...
const mapClassMembers = ({ config, context, template }) => {
   ...
   const _dependencyMembers = dependencyMembers.get(context.contextId);
   for (const { key } of dictionary.filter(x => x.depth === 0)) {
      const memberName = key;
      let types = [];
      let args = {};
      {
         for (const { key, value } of dictionary.filter(x => x.depth === 1)) {
            if (key === 'type') {
      ...
      const member = new Member(memberName, types, args, false, null, context);
      _dependencyMembers.push(member);
   }
}
...
const getConfigDictionary = ({ config, template, depth = 0 }) => {
   if (!template) {
   ...
   for (const key of Object.keys(template)) {
      ...
      dictionary.push({ key, value, depth, Id });
      if (typeof tempValue === 'object') {
         if (Object.keys(tempValue).length > 0) {
            const children = getConfigDictionary({ config: value, template: tempValue, depth: depth + 1 });
            dictionary = dictionary.concat(children);
         }
      }
   }
   return dictionary;
