...
class MemberInfo {
   constructor(name, ctor, args, isPublic, resolve, value, contextName) {
...
const memberCallstack = new WeakMap();
export class Container {
   constructor() {
      const context = this;
      memberCallstack.set(context, []);
      ...
      context.dependency = (config) => {
         if (typeof config === 'function') {
            ...
            const member = new MemberInfo(id, null, config.ctorArgs, false, config, null, context.constructor.name);
            mapDependencyMembers({ context, member });
         } else {
            for (const member of getValueTypeMembers({ config, contextName: context.constructor.name })) {
               mapDependencyMembers({ context, member });
            }
            for (const member of getClassMembers({ config, contextName: context.constructor.name })) {
               mapDependencyMembers({ context, member });
            }
         }
      }
...
const wrapMembersFromPrototype = ({ context }) => {
   for (const wrappedMmemberInfo of getAllPublicMembersFromPrototype({ context })) {
      ...
      contextPrototype[wrappedMmemberInfo.name] = async (args) => {
            ...
            if (isClass || isValueType) {
               if (!contextPrototype[name]) {
                  Object.defineProperty(contextPrototype, name, {
                     configurable: false,
                     get: () => {
                        const stack = memberCallstack.get(context);
                        stack.unshift({ context: member.context, name: member.name, Id: member.Id });
         ...
         const stack = memberCallstack.get(context);
         stack.unshift({ context: wrappedMmemberInfo.context, name: wrappedMmemberInfo.name, Id: wrappedMmemberInfo.Id });
...
const getAllPublicMembersFromPrototype = ({ context }) => {
   ...
   return properties.map((prop) => {
      ...
      return new MemberInfo(prop, member, {}, true, null, null, context.constructor.name);
   });
};

const getClassMembers = ({ config, contextName }) => {
   return Object.keys(config).reduce((items, key) => {
      ...
      if (typeof childConfig === 'object') {
         ...
         if (childConfig.ctorArgs && Class) {
            const member = new MemberInfo(key, Class, childConfig.ctorArgs, false, null, null, contextName);
            items.push(member);
         }
      }
      return items;
...
const getValueTypeMembers = ({ config, contextName }) => {
   return Object.keys(config).reduce((items, key) => {
      const value = config[key];
      if (!value.ctorArgs) {
         const member = new MemberInfo(key, null, null, false, null, value, contextName);
         items.push(member);
      }
      return items;
   }, []);
