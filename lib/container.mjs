...
export class Container extends Reference {
   ...
   constructor(containerConfigNode) {
      ...
      super('container');
      let config = containerConfigNode.find([], 'container');
      let classNames = Object.keys(config);
      if (classNames.length === 0) {
         throw new Error('container could not find any class configuration');
      }
      const removeClassConfig = [];
      for (const className of classNames) {
         const classConfigNode = config[className];
         if (!process.environment.isProduction) {
            const classConfig = classConfigNode.find(['classMock'], classConfigNode.key);
            const mockConfig = classConfig['classMock'];
            if (!utils.isEmptyObject(mockConfig) && !removeClassConfig.find(x => x.key === classConfigNode.key)) {
               let classMockKey = Object.keys(mockConfig)[0];
               let classConfigNodeKey = classConfigNode.key;
               const classMockNode = config[classMockKey];
               classMockNode.key = classConfigNodeKey;
               removeClassConfig.push(classConfigNode);
            }
         }
      }
      for (const classConfigToRemove of removeClassConfig) {
         classConfigToRemove.remove();
      }
      config = containerConfigNode.find([], 'container');
      ...
      for (const classMemberInfo of classMembersInfo) {
         ...
         for (const otherClassMemberInfo of classMembersInfo.filter(cmi => membersInfo.find(mi => mi.Class && mi.Class[cmi.name]))) {
            const membersInfoRefToOtherClass = membersInfo.filter(mi => mi.Class && mi.Class[otherClassMemberInfo.name]);
            for (const memberInfo of membersInfoRefToOtherClass) {
               memberInfo.addReference(otherClassMemberInfo.Id, ClassMemberInfo.prototype);
            }
            if (membersInfoRefToOtherClass.length > 0) {
               classMemberInfo.addReference(otherClassMemberInfo.Id, ClassMemberInfo.prototype);
            }
         }
      }
   }
   ...
   */
   async getReference(name) {
      ...
      const referencedClassMembersInfo = classMemberInfo.getAllReferences(ClassMemberInfo.prototype);
      for (const refClassMemberInfo of referencedClassMembersInfo) {
         this.getReference(refClassMemberInfo.name);
      }
      const classMember = new ClassMember(classMemberInfo.Id);
      await classMember.constructor();
      classMemberInfo.addReference(classMember.Id, ClassMember.prototype);
      return classMember;
   }
}