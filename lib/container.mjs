...
export class Container {
   constructor() {
      ...
      setInterval(async () => {
         ...
         const { member: { context, name, member, args, resolve, value, isAClass }, priority } = stack.shift();
...
const getClassMembers = ({ config, context }) => {
   return Object.keys(config).reduce((items, key) => {
      ...
      if(typeof childConfig === 'object') {
         ...
         const Class = keys.filter(key2 => childConfig[key2] && childConfig[key2].name && childConfig[key2].name.toLowerCase() === key).map(key => childConfig[key])[0]
