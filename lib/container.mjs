...
class MemberProperty {
   ...
   constructor({ member }) {
      ...
      if (member.isPublic) {
         member.context.constructor.prototype[member.name] = async (_args) => {
   ...
   func ({ args, member, value, resolve }) {
      ...
      if (globalCallstackItem.context === context.constructor.name && globalCallstackItem.function === 'func') {
         ...
         if (member.isClass) {
            let instance = null;
            const fakeClass = dependencyMockMembers.get(member.func.name);
            if (fakeClass) {
               instance = new fakeClass(member.args);
               console.log(instance.contextId);
            } else {
               instance = new member.func(member.args);
            }
            return instance;
         }
      }
