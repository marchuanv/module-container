...
class Member extends ContainerMemberCallStack {
   ...
   constructor() {
      ...
      context.dependency = (config) => {
         if (typeof config === 'function') {
            ...
            const member = new Member(id, null, config.ctorArgs, false, config, null, context.constructor.name);
...
const getAllPublicMembersFromPrototype = ({ context }) => {
   ...
   return properties.map((prop) => {
      ...
      return new Member(prop, member, {}, true, null, null, context.constructor.name);
   });
};
   ...
   return Object.keys(config).reduce((items, key) => {
      ...
      if (typeof childConfig === 'object') {
         ...
         if (childConfig.ctorArgs && Class) {
            const member = new Member(key, Class, childConfig.ctorArgs, false, null, null, contextName);
            items.push(member);
         }
      }
      return items;
...
const getValueTypeMembers = ({ config, contextName }) => {
   return Object.keys(config).reduce((items, key) => {
      ...
      if (!value.ctorArgs) {
         const member = new Member(key, null, null, false, null, value, contextName);
         items.push(member);
      }
      return items;
   }, []);
