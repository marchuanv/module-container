...
class Member {
   constructor(name, ctor, args, isPublic, resolve, value, context) {
      ...
      this.value = value;
      this.Id = `${this.context.contextId}-${this.name}`.toLowerCase();
      ...
      if (this.isPublic) {
         ...
         for (const dependantMember of this.dependantMembers.filter(d => d.isClass || d.isPublic)) {
            this.dependantMembers = this.dependantMember.filter(d => d.Id !== dependantMember.Id);
         }
      }
   }
   get isAsync() {
   ...
   async call({ args }) {
      ...
      return await this.func.call(this.context, this.args);
   }
}
...
export class Container {
   ...
   constructor(config) {
      ...
      if (areAllPublicMembersAsync({ context })) {
         for (const member of createPublicMembersFromPrototype({ context })) {
            contextPrototype[member.name] = async (args) => {
...
const areAllPublicMembersAsync = ({ context }) => {
   const members = createPublicMembersFromPrototype({ context });
   const asyncMembers = members.filter(mi => mi.isAsync);
   return members.length === asyncMembers.length;
}

const createPublicMembersFromPrototype = ({ context }) => {
