...
export class Container {
   ...
   async mockClass({ Class, ctorArgs }) {
      const context = this;
      for (const dependencyMember of dependencyMembers.get(context).filter(dep => dep.isClass)) {
         if (Class.name === dependencyMember.func.name) {
            dependencyMember.func = Class;
            dependencyMember.args = ctorArgs;
         }
      }
   }
}
