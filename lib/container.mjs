...
export class Container extends ContainerLogging {
   constructor(config) {
      ...
      if (references.has(ContainerKeys)) {
         references.get(ContainerKeys).push(this);
      } else {
         references.set(ContainerKeys, [this]);
      }
      configure.call(this);
      Object.freeze(this);
   ...
   get singleton() {
      const { singleton } = references.get(this);
      return singleton;
   }

}
references.set(Singletons, []);
references.set(ContainerKeys, []);
const configure = function () {
   let container = references.get(this);
   ...
   if (container.singleton) {
      for (const conKey of references.get(ContainerKeys)) {
         const _con = references.get(conKey);
         if (_con.contextName === container.contextName && _con.contextId !== container.contextId) {
            const existingContainer = references.get(conKey);
            references.delete(container.context);
            references.set(container.context, {
            ...
            container = references.get(container.context);
         }
      }
   }
...
const setupPublicMembers = function (container) {
   ...
   for (const memberName of container.originalMemberNames.filter(name => !memberExlusions.find(excl => excl === name))) {
      const prototype = container.context.constructor.prototype;
