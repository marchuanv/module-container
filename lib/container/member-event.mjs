...
export class MemberEvent {
    constructor({ memberInfo, context }) {
        this.logging = new Logging({ contextId: context.contextId });
    ...
    raise({ timeoutMill, resolve, reject, data }) {
        setTimeout(async () => {
            ...
            try {
                ...
                const output = await callback.call(this.context, { memberInfo, data, logging });
                ...
                if (memberInfo.errorHalt) {
                    logging.log(`'${contextName}.${memberName}' member event raised in ${contextId} context resulted in error: `, error);
                    reject(error);
                } else {
                    logging.log(`'${contextName}.${memberName}' member event raised in ${contextId} context resulted in error: `, error);
