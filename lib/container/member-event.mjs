    ...
    raise({ timeoutMill, resolve, reject, data }) {
        setTimeout(async () => {
            ...
            try {
                ...
                await logging.log(`'${contextName}.${memberName}' member event was raised in contxt of ${contextId}`);
                const output = await callback.call(this.context, { memberInfo, data });
                await logging.log(`'${contextName}.${memberName}' member event was handled by subscriber in context of ${contextId}`);
                resolve(output);
            } catch (error) {
