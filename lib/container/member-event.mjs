    ...
    raise({ timeoutMill, resolve, reject, data }) {
        setTimeout(async () => {
            ...
            try {
                ...
                await logging.log(`'${contextName}.${memberName}' member event was raised in ${contextId} context`);
                const output = await callback.call(this.context, { memberInfo, data });
                await logging.log(`'${contextName}.${memberName}' member event was handled by subscriber in ${contextId} context.`);
                resolve(output);
            } catch (error) {
