    ...
    raise({ timeoutMill, resolve, reject, data }) {
        setTimeout(async () => {
            ...
            try {
                ...
                await logging({ contextId }).log(`'${contextName}.${memberName}' member event raised in ${contextId} context, is delegating control to subscriber.`);
                const output = await callback.call(this.context, { memberInfo, data });
                await logging({ contextId }).log(`'${contextName}.${memberName}' member event raised in ${contextId} context, got response from subscriber.`);
                resolve(output);
            } catch (error) {
                if (memberInfo.errorHalt) {
                    await logging({ contextId }).log(error);
                    reject(error);
                } else {
                    await logging({ contextId }).log(error);
