...
setCallstack([]);
const removeFromCallstack = ({ memberId }) => {
    let callstack = getCallstack();
    callstack = callstack.filter(x => x.memberInfo.memberId !== memberId);
    setCallstack(callstack);
}
const addToCallstack = ({ context, memberInfo }) => {
    getCallstack().unshift({ context, memberInfo });
}