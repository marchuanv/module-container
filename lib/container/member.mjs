      ...
      if (!contextLock.get(contextId)) {
         ...
         const id = setInterval(async () => {
            if (eventQueue.has(contextId)) {
               ...
               const eventsReadyForContext = contextEventQueue
                  .filter(e => e.ready && !e.isRaised && !e.isRaising)
                  .sort((e1, e2) => e1.priority - e2.priority);
               if (eventsReadyForContext.length > 1) {
                  console.log();
               }
...
const createMember = function ({ context, memberInfo }) {
   let raiseEvent = (data) => {
      const priority = eventQueue.has(context.contextId) ? eventQueue.get(context.contextId).length : 1;
      return new Promise((resolve, reject) => { new MemberEvent({ memberInfo, data, promise: { resolve, reject }, context, priority: priority + 1 }); });
   }
   if (memberInfo.isClass || memberInfo.isContainerClass || memberInfo.isValueType) {
