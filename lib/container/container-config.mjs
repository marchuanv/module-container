...
class Node {
    ...
    clone() {
        ...
        const valueType = typeof clonedNode.value;
        if (clonedNode.value instanceof Node) {
            const clonedChildNode = clonedNode.value.clone();
            setPrivatePropertyValue(clonedNode, 'type', Node);
            getPrivatePropertyValue(clonedNode, 'children').push(clonedChildNode);
            setPrivatePropertyValue(clonedNode, 'value', null);
        } else if (Array.isArray(clonedNode.value)) {
            setPrivatePropertyValue(clonedNode, 'type', Array);
            for (const item of clonedNode.value) {
                if (item instanceof Node) {
                    const clonedChildNode = item.clone();
                    setPrivatePropertyValue(clonedNode, 'type', Node);
                    getPrivatePropertyValue(clonedNode, 'children').push(clonedChildNode);
                } else {
                    throw new Error(`value argument type must be an instance(s) of ${Node.name} or primitive type. i.e. string, number or boolean`);
            ...
            setPrivatePropertyValue(clonedNode, 'type', Object);
        } else if (valueType === 'string') { //leaf node
            setPrivatePropertyValue(clonedNode, 'type', String);
        } else if (valueType === 'number') { //leaf node
            setPrivatePropertyValue(clonedNode, 'type', Number);
        } else if (valueType === 'boolean') { //leaf node
            setPrivatePropertyValue(clonedNode, 'type', Boolean);
        } else if (valueType === 'function') { //leaf node
            setPrivatePropertyValue(clonedNode, 'type', Function);
        } else {
            throw new Error(`value argument type must be an instance(s) of ${Node.name}, object or primitive type. i.e. string, number or boolean`);
        }
        for (const child of getPrivatePropertyValue(clonedNode, 'children')) {
            setPrivatePropertyValue(child, 'parent', clonedNode);
        }
        getPrivatePropertyValue(nodeToClone.parent, 'children').push(clonedNode);
    }
    toString() {
        let whiteSpace = ' ';
    ...
    */
    constructor(template, config) {
        ...
        for (const configDictionaryItem of configDictionary.filter(x => x.type === 'object' && x.isAnyKey)) {
            ...
            valueStructureMatchTemplate = configDictionaryItem.value[valueStructureMatchTemplate]
        }
        while (template.nextChild) {
            ...
            valueStructureMatchTemplate = _config !== undefined;
        }
