let _logs = [];
process.on("exit", () => {
    if (process.environment.logging.enabled && process.environment.logging.onExit) {
        Logging.writeLogs(); 
    }
});
let isLogging = false;
export class Logging {
    ...
    async log(message) {
        ...
        if (process.environment.logging.enabled) {
            setTimeout(() => {
                process.environment.logging.onExit = false;
                if (isLogging) {
                    if (!process.environment.logging.onExit) {
                        console.clear();
                        Logging.writeLogs();
                    }
                }
