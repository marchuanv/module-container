...
export class Container {
   constructor(config) {
      ...
      this.config = new ContainerConfig(new ContainerConfigTemplate('root', [new ContainerConfigTemplate('container', [
...
const mapPrivateMembers = function() {
   this.config.find('members', (node) => {
      const { name, value, type, callback, args } = node.value;
      let memberInfo = null;
      if (type && args) {
...
const configureMockedMembers = function() {
   this.config.find('mocks', (node) => {
      const { type, mockType, args } = node.value;
