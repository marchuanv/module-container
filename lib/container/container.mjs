   ...
   getConfig({
      config, configTemplate, callback: ({ singleton }) => {
         ...
         if (singletons.has(contextName)) {
            const singletonSingleton = singletons.get(contextName);
            proxyMembers({ sourceContext: singletonSingleton, destinationContext: context });
            isConfigured = true;
...
const mapPublicMembersFromPrototype = ({ context }) => {
   ...
   const newMembersInfo = memberNames.map((memberName) => {
      ...
      return new MemberInfo(memberName, member, {}, true, null, context);
   });
