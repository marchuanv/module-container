...
const mapPrivateMembers = ({ config, context }) => {
   ...
   getConfig({
      config, configTemplateMatch, callback: ({ name, value, type, func, args }) => {
         let memberInfo = null;
         if (type && args) {
            args.contextId = context.contextId;
            memberInfo = new MemberInfo(name, type, args, false, null);
         } else if (name && value) {
            memberInfo = new MemberInfo(name, null, null, false, value);
         } else if (func && args) {
            memberInfo = new MemberInfo(name, func, args, false, null);
         }
         ...
         for (let i = 0; i < getMembersInfo({ context }).length; i++) {
            ...
            if (dependencyMember.isClass && dependencyMember.isContainerClass &&
               dependencyMember.func.find(f => type.find(x => x.name === f.name))) {
               {
                  ...
                  const memberInfo = new MemberInfo(name, mockType, args, false, null);
                  getMembersInfo({ context }).push(memberInfo);
               }
            }
         }
...
const configureMembersBehaviour = ({ config, context }) => {
   const configTemplateMatch = new ConfigTemplate({ name: 'behaviour' });
   getConfig({
      config, configTemplateMatch, callback: ({ func, errorHalt, errorReturn }) => {
         for (let i = 0; i < getMembersInfo({ context }).length; i++) {
            const memberInfo = getMembersInfo({ context })[i];
            memberInfo.errorReturn = errorReturn;
            ...
            switch (configTemplate.id) {
               case 'members': {
                  if (configTemplate.match({ object })) {
                     for(const childNode of children) {
                        ...
                        const callbackNode = childNode.children.find(x => x.name === 'callback');
                        ...
                        } else if (callbackNode !== undefined && argsNode !== undefined) {
                           callback({ name: childNode.name, func: callbackNode.value.func, args: argsNode.value });
                        } else {
               ...
               case 'behaviour': {
                  if (configTemplate.match({ object })) {
                     ...
                     const errorReturnNode = children.find(x => x.name === 'errorReturn');
                     callback({ 
                        errorHalt: errorHaltNode.value,
                        errorReturn: errorReturnNode.value,
