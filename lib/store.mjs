...
export class Store {
    ...
    async logout() {
        ...
        const { $githubBranch } = container;
        const branchName = session.sessionId;
        session.sessionId = null;
        if ((await $githubBranch.exists({ branchName }))) {
            await $githubBranch.delete({ branchName });
        }
        container.flagAsGarbage({ ref: '$octokitWithDefaults' });
        container.flagAsGarbage({ ref: '$store' });
        container.flagAsGarbage({ ref: '$githubBranch' });
        container.flagAsGarbage({ ref: '$githubFile' });
    }
